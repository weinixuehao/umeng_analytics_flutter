 ...
 */
public class UmengAnalyticsFlutterPlugin implements MethodCallHandler {
    ...
    public static void registerWith(Registrar registrar) {
        final MethodChannel channel =
                new MethodChannel(registrar.messenger(), "umeng_analytics_flutter");
        channel.setMethodCallHandler(new UmengAnalyticsFlutterPlugin(registrar.activity()));
    }
    ...
    /**
     * 计数事件统计
     * 例如：统计微博应用中”转发”事件发生的次数，那么在转发的函数里调用该函数
     * @param call
     * @param result
     */
    public void eventCounts(MethodCall call, Result result) {
        /**
         * 参数1： context	当前宿主进程的ApplicationContext上下文
         * 参数2： eventId	为当前统计的事件ID
         * 参数3： label	 为事件的标签属性
         */
